<!DOCTYPE html>
<html lang="ko"xmlns:th="http://www.thymeleaf.org">

<ul class="mt-3" th:fragment="ul">
	<li class="activity-list d-flex">
		<div class="activity-time pt-2 pe-3 me-3">
			<p class="date m-0">04:45 pm</p>
			<span class="badge shade-yellow">25%</span>
		</div>
		<div class="py-3">
			<h5>Testing</h5>
			<p>by Andrew Seth</p>
			<div class="badge shade-light-red">4 days left</div>
		</div>
	</li>
	<li class="activity-list d-flex">
		<div class="activity-time pt-2 pe-3 me-3">
			<p class="date m-0">05:30 pm</p>
			<span class="badge shade-green">85%</span>
		</div>
		<div class="py-3">
			<h5>Product Launch</h5>
			<p>by Berechiah Philip</p>
			<div class="badge shade-light-blue">
				3 days left
			</div>
		</div>
	</li>
	<li class="activity-list d-flex">
		<div class="activity-time pt-2 pe-3 me-3">
			<p class="date m-0">10:30 am</p>
			<span class="badge shade-red">75%</span>
													</div>
													<div class="py-3">
														<h5>Gear - Admin Dashboard</h5>
														<p>by Elnathan Lois</p>
														<div class="badge shade-light-red">
															8 hours left
														</div>
													</div>
												</li>
												<li class="activity-list d-flex">
													<div class="activity-time pt-2 pe-3 me-3">
														<p class="date m-0">11:30 am</p>
														<span class="badge shade-blue">50%</span>
													</div>
													<div class="py-3">
														<h5>Mobile App</h5>
														<p>by Patrobus Nicole</p>
														<div class="stacked-images my-2">
															<img src="/adm/template/gear/assets/images/user4.png" alt="Admin Dashboards" />
															<img src="/adm/template/gear/assets/images/user4.png" alt="Admin Dashboards" />
															<img src="/adm/template/gear/assets/images/user2.png" alt="Admin Dashboards" />
															<img src="/adm/template/gear/assets/images/user3.png" alt="Admin Dashboards" />
															<span class="plus shade-red">+3</span>
														</div>
													</div>
												</li>
												<li class="activity-list d-flex">
													<div class="activity-time pt-2 pe-3 me-3">
														<p class="date m-0">12:50 pm</p>
														<span class="badge shade-yellow">90%</span>
													</div>
													<div class="py-3">
														<h5>Material Design Kit</h5>
														<p>by Abilene Omega</p>
														<div class="badge shade-light-blue mb-2">
															2 days left
														</div>
														<div class="stacked-images my-2">
															<img src="/adm/template/gear/assets/images/user5.png" alt="Admin Dashboards" />
															<img src="/adm/template/gear/assets/images/user3.png" alt="Admin Dashboards" />
															<img src="/adm/template/gear/assets/images/user2.png" alt="Admin Dashboards" />
															<span class="plus shade-green">+2</span>
														</div>
													</div>
												</li>
												<li class="activity-list d-flex">
													<div class="activity-time pt-2 pe-3 me-3">
														<p class="date m-0">02:30 pm</p>
														<span class="badge shade-green">50%</span>
													</div>
													<div class="py-3">
														<h5>Invoice Design</h5>
														<p>by Shelomi Sarah</p>
														<div class="badge shade-light-blue">
															2 days left
														</div>
													</div>
												</li>
												<li class="activity-list d-flex">
													<div class="activity-time pt-2 pe-3 me-3">
														<p class="date m-0">04:10 pm</p>
														<span class="badge shade-blue">89%</span>
													</div>
													<div class="py-3">
														<h5>Backend Development</h5>
														<p>by Mareshah Nicole</p>
														<div class="stacked-images mt-3">
															<img src="/adm/template/gear/assets/images/user4.png" alt="Admin Dashboards" />
															<img src="/adm/template/gear/assets/images/user3.png" alt="Admin Dashboards" />
															<img src="/adm/template/gear/assets/images/user5.png" alt="Admin Dashboards" />
															<span class="plus shade-primary">+7</span>
														</div>
													</div>
												</li>
												<li class="activity-list d-flex">
													<div class="activity-time pt-2 pe-3 me-3">
														<p class="date m-0">03:45 pm</p>
														<span class="badge shade-red">77%</span>
													</div>
													<div class="py-3">
														<h5>Mobile App</h5>
														<p>by Anaiah Edrei</p>
														<div class="badge shade-light-yellow">
															3 days left
														</div>
													</div>
												</li>
											</ul>
</html>